...
%sendableBits = randi([0 1], 1, read_length);
...
gainSignal = carrierSignal*gainFactor;         %Golay sequence followed by modulated simples. No pulse, SRRC, carrier freq

%Add AWGN based on the SNR and Attenuation Factor!
SNR = (gainFactor^2)*SNR_input;
receivedSignal = awgn(gainSignal, 10*log10(SNR)); %SNR must be in DB

%% Automatic Gain Control (Phat and Joseph)
% Use detected training sequence to estimate the gain here... not the
% actual data!
...
%% Demodulate the data
% Get rid of training sequence before demodulation
...
%Difference between estimatedd and true gain, error squared plotted vs SNR
