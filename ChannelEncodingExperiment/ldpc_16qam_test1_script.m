...
e_message_mod_gray = qammod(e_message,M,'InputType','bit','UnitAveragePower',true);
...
unc_message_mod_gray = qammod(message,M,'InputType','bit','UnitAveragePower',true);
...
for ii = 1 : length(snr)
    ...
    e_message_demod_dec = qamdemod(r_signal_gray,M,'UnitAveragePower',true,'OutputType','llr','NoiseVariance',noiseVar);
    unc_message_demod_dec = qamdemod(unc_r_singal_gray,M,'UnitAveragePower',true,'OutputType','bit','NoiseVariance',noiseVar);
    ...
    message_decode = ldpcDecoder(e_message_demod_dec);
    %Calculate local and total bit error 
    num_err = biterr(message,double(message_decode));
    %total_err = total_err + num_err;
    unc_num_err = biterr(message,double(unc_message_demod_dec));
