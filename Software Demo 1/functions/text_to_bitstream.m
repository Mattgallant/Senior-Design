...
%   Function takes in a value that represents the number of characters to be sent
%   and the text file and converts it to bits until the specified amount is reached

function [sourceCharacters,sendableBits] = text_to_bitstream(filePointer,numberOfCharacters)
    ...
    sourceCharacters= fileValues(1:fix(numberOfCharacters)); %in ascii
    ...
    characterConversion= reshapedBinaryValuesArray(1:numberOfCharacters*7);
    sendableBits= zeros(1,numberOfCharacters*7);    
    zeroValue='0';
    oneValue='1';
    ...
    for i=1:1:numberOfCharacters*7
